...
do NOT add node_modules to git.) -> this issue didn't show up at 42, just on Steffis device.
...
in ./transcendence terminal calling 'make' or 'make all' (or 'make home' for the - version) should build the images and start the containers in detached mode. 

while containers are running: 'docker compose logs frontend' should show two sites (Local and Network). use Local to open the frontend UI. it should show a blueish page with dummy text. 
...
make fcleanhome currently does not touch images and volumes. you will have to delete those manually for now. 
